...
export default function AdminRunesPage() {
    ...
    const handleAddRune = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const runesDocRef = doc(db, 'idealRunes', `tier${tier}`);
            const docSnap = await getDoc(runesDocRef);
    ...
    const handleRemoveRune = async (runeToRemove: IdealRune) => {
        if (!confirm(`Tem certeza que deseja remover a runa "${runeToRemove.name}"?`)) return;
    ...
    const handleUpdateRune = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            // This is a limitation of Firestore array updates.
            await updateDoc(runesDocRef, { runes: arrayRemove(editingRune) });
            await updateDoc(runesDocRef, { runes: arrayUnion(updatedRune) });
            ...
            <main className="flex-1 p-4 md:p-8">
                <div className="mx-auto max-w-4xl">
                     <div className="mb-6 flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-3xl font-bold tracking-tight text-primary">Gerenciar Runas Ideais</h1>
                         ...
                         <div className="ml-auto w-full max-w-[180px]">
                            ...
                            <Select value={String(tier)} onValueChange={handleTierChange}>
                              ...
                              <SelectContent>
                                {Array.from({ length: 7 }, (_, i) => i + 2).map(t => (
